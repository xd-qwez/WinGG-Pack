...
$(document).on("click", ".openinOpenCaseButton", function () {
    ...
    $.post(
        "https://ak4y-caseOpening/caseOpenSelect",
        JSON.stringify({
...
$(document).on("click", "#collectButton", function () {
    ...
    collectedSound.play();
    $.post(
        "https://ak4y-caseOpening/collectItem",
        JSON.stringify({
...
$(document).on("click", "#sellButton", function () {
    ...
    $.post(
        "https://ak4y-caseOpening/sellItem",
        JSON.stringify({
...
$(document).on("click", ".redeemAcceptButton", function () {
    ...
    if (codeInputValue != "REDEEM CODE.." && codeInputValue.length > 0) {
        $.post(
            "https://ak4y-caseOpening/sendInput",
            JSON.stringify({
...
$(document).on("keydown", function () {
    switch (event.keyCode) {
        case 27: // ESC
            if (escUsable) {
                $.post("https://ak4y-caseOpening/closeMenu", JSON.stringify());
                $(".fullBackGround").hide();
                $(".generalSection").hide();
                $(".caseOpeningSection").hide();
                $(".topBar").hide();
