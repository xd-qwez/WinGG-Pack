...
local SettingsButton = {
    Rectangle = { Y = 0, Width = MainSettings.genericMenuWidth, Height = 32 },
    Text = { X = 8, Y = 3.0, Scale = 0.28 },
    LeftBadge = { Y = -2, Width = 40, Height = 40 },
    RightBadge = { X = 425, Y = 1.0, Width = 30, Height = 30 },
    RightText = { X = 450, Y = 4, Scale = 0.22 },
    SelectedSprite = { Dictionary = "commonmenu", Texture = "gradient_nav", Y = 0, Width = MainSettings.genericMenuWidth, Height = 32 },
}
        ...
        if CurrentMenu.Pagination.Minimum <= Option and CurrentMenu.Pagination.Maximum >= Option then
            ...
            if Active then
                if Style.Color and Style.Color.HightLightColor then
                    ...
                    RenderSprite(SettingsButton.SelectedSprite.Dictionary, SettingsButton.SelectedSprite.Texture, CurrentMenu.X, CurrentMenu.Y + SettingsButton.SelectedSprite.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, SettingsButton.SelectedSprite.Width + CurrentMenu.WidthOffset, SettingsButton.SelectedSprite.Height, 0.0, 180, 180, 180, 240)
                end
            end
            if Enabled then
