...
local function saveIdentityToDatabase(identifier, identity)
    MySQL.update.await(
        'UPDATE users SET firstname = ?, lastname = ?, sex = ? WHERE identifier = ?',
        {identity.firstName, identity.lastName, identity.sex, identifier})
end

local function deleteIdentityFromDatabase(xPlayer)
    MySQL.query.await(
        'UPDATE users SET firstname = ?, lastname = ?, sex = ?, skin = ? WHERE identifier = ?',
...
if Config.UseDeferrals then
    AddEventHandler('playerConnecting', function(playerName, setKickReason, deferrals)
        ...
        if identifier then
            MySQL.single('SELECT firstname, lastname, sex FROM users WHERE identifier = ?',
