...
if ESX.GetConfig().Multichar then
	...
	function CharacterOptions(Characters, slots, SelectedCharacter)
		local elements = {{title = "Postać: ".. Characters[SelectedCharacter.value].firstname .. " ".. Characters[SelectedCharacter.value].lastname,icon = "fa-regular fa-user", unselectable = true}, 
		{title = "Powrót", unselectable = false,icon = "fa-solid fa-arrow-left", action = "return"}}
		if not Characters[SelectedCharacter.value].disabled then 
			elements[3] = {title = _('char_play'), icon ="fa-solid fa-play",action = 'play', value = SelectedCharacter.value}
		else
			elements[3] = {title = _('char_disabled'), value = SelectedCharacter.value, icon ="fa-solid fa-xmark"}
		end
		if Config.CanDelete then elements[4] = {title = _('char_delete'),icon ="fa-solid fa-xmark", action = 'delete', value = SelectedCharacter.value} end
	...
	function SelectCharacterMenu(Characters, slots)
		...
		local elements = {{title = "Wybierz postać." , icon = "fa-solid fa-users", unselectable = true}}
	...
	AddEventHandler('esx:playerLoaded', function(playerData, isNew, skin)
		...
		TriggerServerEvent('InteractSound_SV:PlayOnSource', 'gramy', 1.0)
