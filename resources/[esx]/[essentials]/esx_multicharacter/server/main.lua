...
elseif ESX.GetConfig().Multichar == true then
		...
		if result then
			...
			for i = 1, characterCount, 1 do
				...
				if v then
					...
					local job, grade = v.job or 'unemployed', v.job_grade
	...
	MySQL.ready(function()
		...
		if DB_COLUMNS then
			...
			FETCH = 'SELECT identifier, accounts, job, job_grade, firstname, lastname, sex, skin, disabled FROM users WHERE identifier LIKE ? LIMIT ?'
		end
	end)
