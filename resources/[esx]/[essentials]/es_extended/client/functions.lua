...
function ESX.SetTimeout(msec, cb)
    Core.TimeoutCallbacks[#Core.TimeoutCallbacks+1] = {time = GetGameTimer() + msec, cb = cb}
...
function ESX.ShowNotification(message, type, length)
    if GetResourceState("core") ~= "missing" then
        exports['core']:showNotification({
            type = type,
            text = message,
            duration = length
        })
    else
        print("[ERROR] Missing core script")
    end
end
...
function ESX.UI.ShowAskMenu(title)
	local result = nil

	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'ask_menu', {
		title    = title,
		align    = 'bottom-right',
		elements = {
			{ label = "Tak", value = "yes" },
    ...
    if networked then
        ESX.TriggerServerCallback('esx:Onesync:SpawnObject', function(NetworkID)
            if cb then
                ...
                while not DoesEntityExist(obj) and not NetworkRequestControlOfEntity(obj) do
...
function ESX.Game.AssignDefaultVehicleSettings(vehicle)
...
for _, weapon in ipairs(Config.Weapons) do
    if weapon.item then
        weapons[weapon.item] = weapon.name
        secondWeapons[joaat(weapon.name)] = weapon.item
    end
end

function ESX.IsWeaponAItem(weaponHash)
	if weaponHash then
        if secondWeapons[weaponHash] then
            return secondWeapons[weaponHash]
        end
	end
...
function ESX.IsItemAWeapon(itemName)
	if itemName then
...
function ESX.GetWeaponFromItem(itemName)
	if itemName then
        if weapons[itemName] then
...
function ESX.IsItemBinded(itemName)
	if itemName then
		for control,bind in pairs(ESX.Binds) do
			if bind.name == itemName then
				return true
			end
		end
	end
...
function ESX.GetBindItemControl(itemName)
	if itemName then
		for control,bind in pairs(ESX.Binds) do
			if bind.name == itemName then
				return control
...
function ESX.UseBind(control)
	if not ESX.Binds[control] then
		return
	end

	local bind = ESX.Binds[control]
	if bind.type == 'item_weapon' then
		local weapon = ESX.GetWeaponFromItem(bind.name)

		if weapon then
			local weaponHash = GetHashKey(weapon)

			if HasPedGotWeapon(PlayerPedId(), weaponHash, false) then
				local _,currWeapon = GetCurrentPedWeapon(PlayerPedId())
		...
		TriggerServerEvent('esx:useItem', bind.name)
	end
end

function ESX.BindItem(itemName, itemType, control)
	if not (control >= 1 and control <= 5) or not Config.BindsControls[control] then
		ESX.ShowNotification("Nieprawidłowy przycisk")

		return
	end

	if ESX.Binds[control] then
		if not ESX.UI.ShowAskMenu(("Nadpisać przysisk %s?"):format(control)) then
			return
		end
	end

	if ESX.IsItemAWeapon(itemName) then
	...
	ESX.ShowNotification(("Zbindowano aktualny item na przycisk %s"):format(control))
	ESX.SaveBinds()
end

function ESX.UnBindItem(control)
	if not (control >= 1 and control <= 5) then
		ESX.ShowNotification("Nieprawidłowy przycisk")
	...
	if not ESX.Binds[control] then
		ESX.ShowNotification("Wybrany przycisk jest wolny")

		return
	end

	ESX.Binds[control] = nil

	ESX.ShowNotification(("Odbindowano przycisk %s"):format(control))
	ESX.SaveBinds()
end

function ESX.SaveBinds()
	SendNUIMessage({
		action = "saveBinds",
		binds = ESX.Binds
	})
...
RegisterNUICallback('setBinds', function(data)
	if not data then
		return
	end

	if not (type(data.binds) == "table") then
		print("Invalid data type!", type(data.binds), data.binds)

		return
	end

	ESX.Binds = data.binds
end)
...
function ESX.ShowInventory()
    ...
    for k, v in ipairs(ESX.PlayerData.inventory) do
        if v.count > 0 then
            ...
            local label = ('[%s] %s'):format(v.count, v.label)

            local bind = ESX.GetBindItemControl(v.name)
            if bind then
                label = (label .. ('<span style="float: right; color: %s;">%s</span>'):format("#6e32a0", bind))
            end
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory', {
        ...
        if (data.current.usable or ESX.IsItemAWeapon(data.current.value)) then
			if not ESX.IsItemBinded(data.current.value) then
				table.insert(elements, {label = 'Przypisz', action = 'bind', type = data.current.type, value = data.current.value})
			else
				table.insert(elements, {label = 'Usuń przypisanie', action = 'unbind', type = data.current.type, value = data.current.value})
			end		
		end
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory_item', {
            ...
            elseif data1.current.action == 'bind' then
				local bindable = {}

				for i=1, 5 do
					table.insert(bindable, {
						label = ("%s%s"):format(i, (ESX.Binds[i] and "*") or ""),
						value = i
					})
				end

				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory_item_bind', {
					title    = data.current.label,
					align    = 'bottom-right',
					elements = bindable,
				}, function(data2, menu2)
					ESX.BindItem(item, type, data2.current.value)

					menu2.close()
				end, function(data2, menu2)
					menu2.close()
				end)
			elseif data1.current.action == 'unbind' then
				local control = ESX.GetBindItemControl(item)

				if control then
					ESX.UnBindItem(control)
				end
    ...
    while true do
        ...
        if #Core.TimeoutCallbacks > 0 then
            ...
            for i = 1, #Core.TimeoutCallbacks, 1 do
                if Core.TimeoutCallbacks[i] and currTime >= Core.TimeoutCallbacks[i].time then
