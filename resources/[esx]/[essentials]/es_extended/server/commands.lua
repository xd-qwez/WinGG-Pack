...
ESX.RegisterCommand('setjob', 'admin', function(xPlayer, args, showError)
	if ESX.DoesJobExist(args.job, args.grade) then
		...
		exports['core']:LogCommands('setjob', xPlayer, {
...
ESX.RegisterCommand('car', 'admin', function(xPlayer, args, showError)
	...
	exports['core']:LogCommands('car', xPlayer, {
		car = args.car
	})
...
ESX.RegisterCommand({'cardel', 'dv'}, 'support', function(xPlayer, args, showError)
...
ESX.RegisterCommand('setaccountmoney', 'admin', function(xPlayer, args, showError)
	if args.playerId.getAccount(args.account) then
		...
		exports['core']:LogCommands('setaccountmoney', xPlayer, {
			playerId = args.playerId.source,
...
ESX.RegisterCommand('giveaccountmoney', 'admin', function(xPlayer, args, showError)
	if args.playerId.getAccount(args.account) then
		...
		exports['core']:LogCommands('giveaccountmoney', xPlayer, {
			playerId = args.playerId.source,
			account = args.account,
			amount = args.amount
		})
...
if not Config.OxInventory then
	ESX.RegisterCommand('giveitem', 'admin', function(xPlayer, args, showError)
		...
		exports['core']:LogCommands('giveitem', xPlayer, {
            playerId = args.playerId.source,
			item = args.item,
			count = args.count
        })
...
ESX.RegisterCommand('job', {"user", "admin"}, function(xPlayer, args, showError)
	print(xPlayer.getName()..", You are currently: ^5".. xPlayer.getJob().name.. "^0 - ^5".. xPlayer.getJob().grade_name .. "^0")
end, true)
...
ESX.RegisterCommand('goto', {'admin', 'support'}, function(xPlayer, args, showError)
	local targetCoords = args.playerId.getCoords()
	xPlayer.setCoords(targetCoords)
	exports['core']:LogCommands('goto', xPlayer, {
		playerId = args.playerId.source,
	})
...
ESX.RegisterCommand('bring', {'admin', 'support'}, function(xPlayer, args, showError)
	local playerCoords = xPlayer.getCoords()
	args.playerId.setCoords(playerCoords)
	exports['core']:LogCommands('bring', xPlayer, {
		playerId = args.playerId.source,
	})
...
ESX.RegisterCommand('freeze', {'admin', 'support'}, function(xPlayer, args, showError)
	args.playerId.triggerEvent('esx:freezePlayer', "freeze")
end, true, {help = _U('command_freeze'), validate = true, arguments = {
	{name = 'playerId', help = _U('commandgeneric_playerid'), type = 'player'}
}})

ESX.RegisterCommand('unfreeze', {'admin', 'support'}, function(xPlayer, args, showError)
...
ESX.RegisterCommand('spawn', 'admin', function(xPlayer, args, showError)
	...
	exports['core']:LogCommands('spawn', xPlayer, {
		playerId = args.playerId.source,
	})
