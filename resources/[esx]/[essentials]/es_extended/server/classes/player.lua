function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, loadout, name, coords)
	...
	function self.removeInventoryItem(name, count, metadata, slot, hideNotification)
		local item = self.getInventoryItem(name)
		...
		if item then
			...
			if newCount >= 0 then
				...
				self.triggerEvent('esx:removeInventoryItem', item.name, item.count, hideNotification)
			end
		end
	end
	...
	function self.setJob(job, grade)
		...
		if ESX.DoesJobExist(job, grade) then
			...
			self.job.grade_permissions = gradeObject.permissions
	...
	function self.showNotification(msg, Ntype, length)
		self.triggerEvent('esx:showNotification', msg, Ntype, length)
	end
