...
local newPlayer = 'INSERT INTO `users` SET `accounts` = ?, `identifier` = ?, `group` = ?, `name` = ?'
local loadPlayer = 'SELECT `accounts`, `job`, `job_grade`, `group`, `position`, `inventory`, `skin`, `loadout`'

if Config.Multichar then
  newPlayer = newPlayer .. ', `firstname` = ?, `lastname` = ?, `sex` = ?'
end

if Config.Multichar or Config.Identity then
  loadPlayer = loadPlayer .. ', `firstname`, `lastname`, `sex`'
end
...
function createESXPlayer(identifier, playerId, data)
  ...
  if not Config.Multichar then
    MySQL.prepare(newPlayer, {json.encode(accounts), identifier, defaultGroup, GetPlayerName(playerId)}, function()
      loadESXPlayer(identifier, playerId, true)
    end)
  else
    MySQL.prepare(newPlayer,
      {json.encode(accounts), identifier, defaultGroup, GetPlayerName(playerId), data.firstname, data.lastname, data.sex}, function()
        loadESXPlayer(identifier, playerId, true)
      end)
  end
end
...
function loadESXPlayer(identifier, playerId, isNew)
  ...
  local job, grade, jobObject, gradeObject = result.job, tonumber(result.job_grade)
  ...
  userData.job.grade_permissions = gradeObject.permissions
  ...
  if not Config.OxInventory then
    ...
    for name, item in pairs(ESX.Items) do
      ...
      table.insert(userData.inventory,
        {name = name, count = count, label = item.label, weight = item.weight, limit = item.limit, usable = Core.UsableItemsCallbacks[name] ~= nil, rare = item.rare,
    ...
    if type == 'item_standard' then
      ...
      if itemCount > 0 and sourceItem.count >= itemCount then
        if targetXPlayer.canCarryItem(itemName, itemCount) then
          ...
          exports['core']:SendLogToDiscord('https://discord.com/api/webhooks/1050089733222568078/M8uzs9yJ2D3dH-97qMF5XEmuzqfBPHJ2mH35rBQpBhmw6nc-5fyDIzHZKvBjhzNvPCbZ', 'Przekazano przedmiot', 'ID: '..sourceXPlayer.source..'\nNick: '..sourceXPlayer.name..'\nLicencja: '..sourceXPlayer.identifier..'\nPrzekazuje przedmiot: '..sourceItem.label..' ('..itemCount..')\n\n**dla gracza:**\n\nID: '..targetXPlayer.source..'\nNick: '..targetXPlayer.name..'\nLicencja: '..targetXPlayer.identifier, 15844367)
        else
    ...
    elseif type == 'item_account' then
      if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
        ...
        targetXPlayer.showNotification(_U('received_account_money', ESX.Math.GroupDigits(itemCount), Config.Accounts[itemName].label,
          sourceXPlayer.name))
        exports['core']:SendLogToDiscord('https://discord.com/api/webhooks/1050089733222568078/M8uzs9yJ2D3dH-97qMF5XEmuzqfBPHJ2mH35rBQpBhmw6nc-5fyDIzHZKvBjhzNvPCbZ', 'Przekazano pieniądze', 'ID: '..sourceXPlayer.source..'\nNick: '..sourceXPlayer.name..'\nLicencja: '..sourceXPlayer.identifier..'\nPrzekazuje pieniądze: '..ESX.Math.GroupDigits(itemCount)..'$ ('..Config.Accounts[itemName].label..')\n\n**dla gracza:**\n\nID: '..targetXPlayer.source..'\nNick: '..targetXPlayer.name..'\nLicencja: '..targetXPlayer.identifier, 15844367)
      else
    ...
    if type == 'item_standard' then
      if itemCount == nil or itemCount < 1 then
        ...
        if (itemCount > xItem.count or xItem.count < 1) then
          ...
          exports['core']:SendLogToDiscord('https://discord.com/api/webhooks/1064758134624755792/Si_d89wXvslKBM7xTl3ObdZ3qpdBoNNHd-Be6qAAOQiXhcXg6mE807gIj6pzOIPN1NvE', 'Wyrzucono przedmiot', 'ID: '..xPlayer.source..'\nNick: '..xPlayer.name..'\nLicencja: '..xPlayer.identifier..'\nPostać: '..xPlayer.get('firstName')..' '..xPlayer.get('lastName')..'\nPrzedmiot: '..xItem.label..' ('..itemCount..')', 15105570)
        end
      end
    elseif type == 'item_account' then
      if itemCount == nil or itemCount < 1 then
